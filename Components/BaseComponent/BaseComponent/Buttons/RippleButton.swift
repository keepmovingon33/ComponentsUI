...
open class RippleButton: UIButton {
    ...
    // set color for ripple
    @IBInspectable open var rippleColor: UIColor = UIColor(white: 1, alpha: 0.5) {
        didSet {
            rippleView.backgroundColor = rippleColor
        }
    }
    
    @IBInspectable open var rippleBackgroundColor: UIColor = BaseColor.main {
        didSet {
    ...
    fileprivate func setupRippleView() {
        var size: CGFloat = bounds.width * CGFloat(ripplePercent)
